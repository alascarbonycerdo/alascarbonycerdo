...
import type { InventoryItem, DaySummary } from '#shared/types/dashboard'
...
const todayRevenueThousands = computed(() => pedidos.value.reduce((sum, pedido) => sum + pedido.totalMiles, 0))
const todayItemsSold = computed(() => pedidos.value.reduce((sum, pedido) => sum + pedido.itemsCount, 0))
...
<template>
  <div class="space-y-8">
    ...
    <div class="grid grid-cols-3 gap-3">
      <StatTile label="Ingresos de hoy" :value="formatCOP(todayRevenueThousands)" />
      <StatTile label="Platos vendidos" :value="String(todayItemsSold)" />
      <StatTile
        label="Stock bajo"
    ...
    <div class="space-y-8" :class="{ 'opacity-60': pending }">
      ...
      <section class="space-y-3">
        <h3 class="font-display text-xl text-gold">Pedidos de hoy</h3>
        <ul v-if="sortedPedidos.length" class="divide-y divide-gold/10 overflow-hidden rounded-2xl bg-ink-soft/40 ring-1 ring-gold/10">
          <li v-for="pedido in sortedPedidos" :key="pedido.id" class="flex items-center justify-between px-4 py-2.5 text-sm">
        ...
        <p v-else-if="!pending" class="text-sm text-gold-soft/50">Aún no hay pedidos hoy en este punto.</p>
      </section>
